...
<body>
        ...
        <div id="dashboard" class="dashboard office-mode" style="display:none;">
            ...
            <div class="map-container tech-detail">
                ...
                <div class="map-wrapper">
                    <div class="map-panel"><div id="map" style="height:400px; border-radius:16px;"></div>
            ...
            <!-- Route Planner -->
            <div class="chart-container tech-detail" id="routePlannerCard">
    ...
    <script>
        // ============================================================
        // GENERIC FIELD ASSET DASHBOARD – NO COMPANY-SPECIFIC REFERENCES
        // ============================================================
        ...
        function getSiteId(r) { return (r.Store || '').replace(/[^a-zA-Z0-9]/g, '_') + '_' + (r.City || '').replace(/[^a-zA-Z0-9]/g, '_'); }
        ...
        function isServiceOverdue(r) { let days = getDaysSinceService(r); return days !== null && days > SERVICE_OVERDUE_DAYS; }
        ...
        function getVolumePriority(r) { return volPriorityMap.get(getSiteId(r)) || 'Low'; }
        ...
        function getVolumeTrend(r) {
            let v1 = getVolume(r), v7 = getVolume7DayAvg(r);
            if (v7 === 0) return 'flat';
            let ch = ((v1 - v7) / v7) * 100;
            if (ch > 5) return 'up';
            if (ch < -5) return 'down';
        ...
        // ===== NOTES =====
        function getNotesKey(id) { return 'asset_notes_' + id; }
        function loadNotes(id) {
            let s = localStorage.getItem(getNotesKey(id));
            if (!s) return [];
            try { return JSON.parse(s).filter(n => new Date(n.date).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000); } catch (e) { return []; }
        }
        function saveNote(id, txt) {
            if (!txt.trim()) return;
            let notes = loadNotes(id);
            notes.unshift({ id: Date.now().toString(), text: txt.trim(), date: new Date().toISOString(), formattedDate: new Date().toLocaleString() });
        ...
        function openNotesModal(id, name) {
            ...
            else div.innerHTML = notes.map(n => `<div><strong>${escapeHtml(n.text)}</strong><br><small>${escapeHtml(n.formattedDate)}</small></div>`).join('');
        ...
        function refreshNotesModal(id) {
            let asset = allData.find(r => getSiteId(r) === id);
            openNotesModal(id, asset ? asset.Store : 'Asset');
        }
        ...
        function updateMapHighlights() {
            if (!map) return;
            map.eachLayer(layer => {
                if (layer instanceof L.CircleMarker && layer.options.siteId) {
                    let sid = layer.options.siteId;
                    if (selectedAssets.has(sid)) {
        ...
        function initMap() {
            ...
            filtered.forEach(r => {
                ...
                if (!isNaN(lat) && !isNaN(lng) && lat !== 0 && lng !== 0) {
                    ...
                    let sid = getSiteId(r);
                    let sel = selectedAssets.has(sid);
                    ...
                    marker.on('click', () => {
                        if (selectedAssets.has(sid)) { selectedAssets.delete(sid); marker.setStyle({ fillColor: col, weight: 2 }); }
                        else { selectedAssets.add(sid); marker.setStyle({ fillColor: '#3b82f6', weight: 3 }); }
                        updateTableCheckboxes();
                        updateSelectAllCheckbox();
                        updateMapHighlights();
                    });
                    marker.bindPopup(`<b>${escapeHtml(r.Store)}</b><br>${escapeHtml(r.Address)}<br>Loss: $${calcDailyLoss(r).toFixed(0)}`);
                    bounds.push([lat, lng]);
                }
            });
        ...
        function refreshSelectionPanel() {
            ...
            if (!selectedAssets.size) { container.innerHTML = 'None selected. Click markers or checkboxes.'; return; }
            let html = '';
            for (let sid of selectedAssets) {
                const k = allData.find(r => getSiteId(r) === sid);
                if (k) {
                    html += `<div class="selected-item">
                        <span><strong>${escapeHtml(k.Store)}</strong><br><small>${escapeHtml(k.Address||'')}</small></span>
                        <button onclick="selectedAssets.delete('${sid}'); toggleAssetSelection('${sid}', false);">✕</button>
                    </div>`;
                }
            }
        ...
        function updateSelectedSummary() {
            ...
            if (count) {
                const withCoords = [...selectedAssets].filter(sid => {
                    const k = allData.find(r => getSiteId(r) === sid);
                    return k && k.Latitude && k.Longitude;
                }).length;
                el.textContent = `🟢 ${count} asset${count>1?'s':''} selected | 📍 ${withCoords} with coordinates | 🗺️ Ready to route`;
            } else {
                el.textContent = '';
            }
        }
        ...
        function parseCSV(text) {
            ...
            allData = data.map(row => ({
                Store: row.Store || row['Site Name'] || 'Unknown',
                Address: row.Address || '',
                City: row.City || '',
                Province: row.State || row.Province || '',
                KioskState: row['Asset State'] || row.State || 'Attract',
                Tx1Day: row['1 Day Activity'] || row.Volume || '0',
                Tx7Day: row['7 Day Activity'] || '0',
                Reject7Day: row['7 Day Failure %'] || row.Reject7Day || row['1 Day Failure %'] || '0',
                Uptime1Day: row['1 Day Uptime %'] || '100',
                Uptime7Day: row['7 Day Uptime %'] || row.Uptime || '100',
                BinFull: row['Total Capacity %'] || row['Capacity %'] || '0',
                LastPM: row['Last Service Date'] || row['Last PM Date'] || '',
                Phone: row['Phone Number'] || row.Phone || '',
                TEMM7Day: row['7 Day Error'] || row.TEMM || '0',
                VPO: row.VPO || row.Supply || '',
                LastTXDate: row['Last Activity Date'] || '',
                Version: row.Version || '',
        ...
        function applyPriorityFilter(data, type) {
            ...
            if (type === 'paper-out') return data.filter(r => { let a = getPaperAlert(r); return a.includes('🟡') || a.includes('🔴'); });
        ...
        function sortData(d) {
            return [...d].sort((a, b) => {
                ...
                switch (currentSort.column) {
                    ...
                    case 'volPriority': va = priorityOrder[getVolumePriority(a)] || 0; vb = priorityOrder[getVolumePriority(b)] || 0; break;
                    case 'traffic': va = trafficOrder[getTrafficLevel(a)] || 0; vb = trafficOrder[getTrafficLevel(b)] || 0; break;
                    case 'trend': va = ((getVolume(a) - getVolume7DayAvg(a)) / (getVolume7DayAvg(a) || 1)) * 100; vb = ((getVolume(b) - getVolume7DayAvg(b)) / (getVolume7DayAvg(b) || 1)) * 100; break;
                    ...
                    default: va = calcDailyLoss(a); vb = calcDailyLoss(b);
                }
        ...
        function updateTableCheckboxes() {
            document.querySelectorAll('.asset-checkbox').forEach(cb => {
                let sid = cb.getAttribute('data-siteid');
                cb.checked = selectedAssets.has(sid);
                let row = cb.closest('tr');
                if (row) row.classList.toggle('selected-row', selectedAssets.has(sid));
            });
            updateSelectAllCheckbox();
            refreshSelectionPanel();
            updateSelectedSummary();
        ...
        function updateSelectAllCheckbox() {
            ...
            if (!sa) return;
            let cbs = document.querySelectorAll('.asset-checkbox');
        ...
        function selectAllAssets() {
            ...
            document.querySelectorAll('.asset-checkbox').forEach(cb => {
                cb.checked = chk;
                let sid = cb.getAttribute('data-siteid');
                if (chk) selectedAssets.add(sid);
                else selectedAssets.delete(sid);
                let row = cb.closest('tr');
                if (row) row.classList.toggle('selected-row', chk);
            });
            updateMapHighlights();
        }
        window.toggleAssetSelection = function(sid, chk) {
            if (chk) selectedAssets.add(sid);
            else selectedAssets.delete(sid);
            const row = document.querySelector(`.asset-checkbox[data-siteid="${sid}"]`)?.closest('tr');
            if (row) row.classList.toggle('selected-row', chk);
            updateMapHighlights();
            updateSelectAllCheckbox();
        };
        ...
        function updateProvinceChart(data) {
            ...
            lossChart = new Chart(document.getElementById('lossChart'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Rejected', data: sorted.map(p => pR[p.prov] || 0), backgroundColor: '#dc2626' },
                        { label: 'Downtime', data: sorted.map(p => pD[p.prov] || 0), backgroundColor: '#f97316' }
                    ]
                },
                options: {
                    responsive: true,
        ...
        function updateTopLossList(data) {
            ...
            cont.innerHTML = sorted.map((r, idx) => `
                <div class="top-loss-item" onclick="document.getElementById('searchInput').value='${escapeHtml(r.Store)}';renderAll();">
                    <div><strong>#${idx+1} ${escapeHtml(r.Store)}</strong><br><small>${getVolume(r)} activities/day</small></div>
        ...
        function updatePriorityButtonCounts() {
            ...
            const counts = {
                ...
                'paper-out': filtered.filter(r => { let a = getPaperAlert(r); return a.includes('🟡') || a.includes('🔴'); }).length,
            ...
            const labels = {
                all: '📊 All',
                urgent: '🔥 High Impact',
                'high-reject': '⚠️ High Failure',
                'full-bin': '🗑️ Full Capacity',
                'paper-out': '📄 Supply Low',
        ...
        function renderAll() {
            ...
            if (currentMode === 'office') {
                ...
                document.getElementById('kpiGrid').innerHTML = `
                    ...
                    <div class="kpi-card"><div class="kpi-label">📊 High Activity</div><div class="kpi-value">${highVolCount}</div></div>
            ...
            document.getElementById('alertBar').innerHTML = `
                <span>⚠️ ${crit} critical | ${warn} warning | ${off} offline | ${binA} full capacity | ${highVolCount} high activity</span>
            ...
            tbody.innerHTML = f.map(r => {
                ...
                const statusClass = getStatusLevel(r) === 'Critical' ? 'critical-row' : getStatusLevel(r) === 'Warning' ? 'warning-row' : '';
                return `<tr class="${statusClass} ${selectedAssets.has(sid)?'selected-row':''}">
                    <td><input type="checkbox" class="asset-checkbox" data-siteid="${escapeHtml(sid)}" ${selectedAssets.has(sid)?'checked':''} onclick="toggleAssetSelection('${escapeHtml(sid)}', this.checked)"></td>
        ...
        function populateFilters() {
            ...
            document.getElementById('provinceFilter').innerHTML = '<option value="all">All</option>' + regions.sort().map(p => `<option value="${p}">${p}</option>`).join('');
            document.getElementById('zoneFilter').innerHTML = '<option value="all">All</option>' + zones.sort().map(z => `<option value="${z}">${z}</option>`).join('');
            ['provinceFilter', 'zoneFilter', 'riskFilter', 'volumePriorityFilter'].forEach(id => {
        ...
        function optimizeRoute(start, stops) {
            ...
            let unv = [...stops], route = [], cur = start;
            while (unv.length) {
                let idx = 0, dist = haversine(cur.lat, cur.lon, unv[0].coord.lat, unv[0].coord.lon);
        ...
        async function planRoute() {
            ...
            if (selectedAssets.size === 0) return showToast('Select assets');
            let start = await geocode(addr);
            if (!start) return showToast('Address not found');
            let stops = [];
            for (let sid of selectedAssets) {
                let k = allData.find(r => getSiteId(r) === sid);
            ...
            document.getElementById('routeStopsList').innerHTML = opt.map((s, i) =>
                `<li>${escapeHtml(s.name)}<br><small>${escapeHtml(s.address)}</small></li>`
            ).join('');
        ...
        function openRouteInMaps() {
            ...
            let url = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(currentRoute.startAddress)}&destination=${encodeURIComponent(lastStop)}`;
                ...
                if (file.name.endsWith('.csv')) {
                    ...
                    reader.onload = ev => {
                        if (parseCSV(ev.target.result)) {
                            ...
                            document.getElementById('dataTimestamp').innerHTML = `📅 Data as of: ${new Date().toLocaleString()} — ${allData.length} assets`;
                            populateFilters();
                            setupSorting();
        ...
        document.getElementById('selectAllCheckbox')?.addEventListener('change', selectAllAssets);
    ...
    try:
        ...
        try:
            ...
            if text and len(text.strip()) > 10:
                return jsonify({'text': f"Image: {file_name}\n\nOCR Extracted Text:\n{text}"})
        except Exception as e:
            print(f"OCR Error: {e}")

        # Fallback: Use GPT-4o-mini vision
        try:
            import requests
                    ...
                    "messages": [
                        {
                            ...
                            "content": [
                                {"type": "text",
                                 "text": "Describe this image in detail. If it contains text, extract it verbatim. What do you see?"},
                                {"type": "image_url", "image_url": f"data:image/png;base64,{file_content}"}
                            ]
                        }
                    ],
                    "max_tokens": 800,
        ...
        return jsonify(
            {'text': f"Image: {file_name}\n\nImage uploaded successfully. (No text could be extracted automatically.)"})
...
def chat():
    ...
    try:
        if fusion:
            ...
            tools = [{"type": "openrouter:web_search"}] if web_search else []
            payload = {
                "model": "deepseek/deepseek-v4-pro-0813",
                "messages": messages,
                "tools": tools,
        ...
        if not result or 'choices' not in result or not result['choices']:
            return jsonify({'error': 'AI service returned an empty response. Please try again.'})

        bot_reply = result['choices'][0]['message']['content']

        if not bot_reply or bot_reply is None:
            fallback = "Hmm, I couldn't find a good answer for that. Try rephrasing or ask me something else! 😊"
            return jsonify({
                'reply': fallback,
                'html_reply': f"<p>{fallback}</p>",
                'total_tokens': total_tokens_used,
                'total_cost': total_cost_usd,
                'message_id': None
            })
        ...
        allowed_tags = ['p', 'br', 'strong', 'em', 'u', 'h1', 'h2', 'h3', 'h4', 'ul', 'ol', 'li', 'blockquote', 'pre',
                        'code', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'a', 'img', 'span', 'div']
        html_reply = bleach.clean(html_reply, tags=allowed_tags, strip=True)
        ...
        if 'messages' not in chat_histories:
            chat_histories['messages'] = {}
        chat_histories['messages'][message_id] = {
            'content': bot_reply,
